...
> ⚠️ **本文件已被新版本替代，仅作历史参考保留，请勿用作当前接入文档。**
>
> **最新 skill 包**：`LabMemory 决策记忆/labmemory-decision-memory/SKILL.md`
> （相对仓库根：`LabMemory 决策记忆/labmemory-decision-memory/SKILL.md`）
>
> 新版本相对本旧版的主要差异：
> 1. 新增「第 0 步：首次接入引导」—— Aily 自助接入 MCP + 自动建「会议纪要入档」事件自动化 + 「状态轮询」interval 任务
> 2. 新增 5 类飞书卡片完整 JSON 模板（复核提醒 / 审计阻断 / 执行偏差 / 知识发布 / 复验提醒）
> 3. 新增状态轮询（后半段链路感知）完整流程
> 4. 新增 16 条实战踩坑记录（接入验证 / MCP 调用 / 卡片推送 / 自动化 / 轮询）
> 5. 凭据（URL 含 token、轮询账号）不再硬编码，改走管理员维护的飞书云文档，绕开 SkillHub 内容安全扫描
>
> 10 个 MCP 工具、`/mcp/sse` 协议、Webhook 推送契约等**基础设施未变**，新 skill 与本旧版共用。
> 接入前请以新 SKILL.md 为准。
...
> **已知限制**：Aily 后台 URL 校验可能拒绝带 queryParam 的输入（报"请输入合法的 URL"）。
> 实测中我们遇到两种情况都能跑通（带 token 拼 URL / 裸 URL + 后端无鉴权）。
> **首选是带 token 拼 URL 的方案**——服务端 `MCP_REQUIRE_AUTH=true` 强制鉴权；
> 若 Aily 后台校验拒掉，回退方案见 A6（裸 URL + `MCP_REQUIRE_AUTH=false`，需 nginx
> IP rate limit 防滥用）。
...
> 鉴权通过 URL 里的 `?token=` 完成。**也不要**担心 Aily 后续调用工具时会丢
> token——MCP 协议设计上 token 只在 SSE 握手时校验一次，后续消息通过 SDK 自动
> 分发的 UUID v4 session_id 路由（128 位熵）。
...
## A6. 如果 Aily 后台报"请输入合法的 URL"（备用方案）

Aily 后端 URL 校验可能拒绝带 `?token=...` 的输入。备用方案：
- 服务端临时切到 `MCP_REQUIRE_AUTH=false`（详见 `AILY_MCP.md §2.2`）
- Aily 后台填裸 URL `https://<your-domain.com>/mcp/sse`
- 公网防护走 nginx `limit_req`（已配 `burst=40 nodelay`）
