...
- THEN 系统 SHALL 将查询改写为含「EXP-DEMO-001」与「浓度」的自包含查询（如「EXP-DEMO-001 的浓度是多少」），用该查询做 BM25/向量召回，`retrieval_details.search_query` 透传改写后查询
...
- GIVEN 会话 S1 历史仅提及 EXP-001
- WHEN 用户追问「它的浓度」
- THEN 改写后查询 SHALL 仅含 EXP-001 相关实体，MUST NOT 引入 EXP-002 等历史未提及的实验
...
凡是已执行检索的轮次（无论最终回答成功或拒答），`retrieval_details` SHALL 一致透传 `vector_available`，不得仅在成功路径返回。`retrieval_details` SHALL 透传 `search_query`（实际用于 BM25/向量召回的查询，可能经查询改写与原始问题不同）。
...
- GIVEN 用户做指代追问，查询被改写
- THEN `retrieval_details.search_query` SHALL 为改写后的自包含查询，前端 SHALL 展示「检索查询」行让用户看到指代消解过程
